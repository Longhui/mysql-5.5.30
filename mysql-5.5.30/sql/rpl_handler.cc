...
Binlog_relay_SQL_delegate *binlog_relay_sql_delegate;
...
Vsr_master_delegate::before_recover(char *fname)
{
  if (likely(NULL != observer))
  {
    ...
    if ( NULL == ha_partner_host ||
         ...
         NULL == ha_partner_password )
    {
      sql_print_warning("Skiped VSR_HA function before_recover() "
                          "for one ha_parter_* isn't set");
      return;
    }
...
Vsr_slave_delegate::master_request(NET *net)
{
  if (likely(NULL != observer))
  {
    ...
    param.net= net;
    param.filename= active_mi->master_log_name;
    param.pos= active_mi->master_log_pos;
...
int delegates_init()
{
  ...
  static my_aligned_storage<sizeof(Binlog_relay_SQL_delegate),
                            MY_ALIGNOF(long)> relay_sql_mem;
#endif
  ...
  void *place_relay_sql_mem= relay_sql_mem.data;
  ...
  binlog_relay_sql_delegate= new (place_relay_sql_mem) Binlog_relay_SQL_delegate;

  if (!binlog_relay_sql_delegate->is_inited())
  {
    sql_print_error("Initialization binlog relay SQL delegates failed. "
                    "Please report a bug.");
    return 1;
  }
#endif
...
void Binlog_relay_SQL_delegate::init_param(Binlog_relay_SQL_param * param,Relay_log_info * rli)
{
  param->master_log_name= rli->group_master_log_name;
  param->master_log_pos= rli->group_master_log_pos;
}

int Binlog_relay_SQL_delegate::after_apply_event(THD * thd, Relay_log_info * rli)
{
  Binlog_relay_SQL_param param;
  init_param(&param, rli);

  int ret= 0;
  FOREACH_OBSERVER(ret, after_apply_event, thd, (&param));
  return ret;
}
...
int register_binlog_relay_sql_observer(Binlog_relay_SQL_observer *observer, void *p)
{
  return binlog_relay_sql_delegate->add_observer(observer, (st_plugin_int *)p);
}

int unregister_binlog_relay_sql_observer(Binlog_relay_SQL_observer *observer, void *p)
{
  return binlog_relay_sql_delegate->remove_observer(observer, (st_plugin_int *)p);
}
