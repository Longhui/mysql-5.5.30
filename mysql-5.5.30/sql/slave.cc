...
/**
  Execute a SHOW SLAVE SQL_THREAD statement.
...
start:
  ...
  */
  ...
  mysql_mutex_unlock(&rli->data_lock);
  if (rli->cur_log->error < 0 && read_retries < 5)
  {
    read_retries++;
...
Stopping slave I/O thread due to out-of-memory error from master");
	  ...
	  DBUG_EXECUTE_IF("crash_after_queue_event", DBUG_SUICIDE(););
...
static int process_io_rotate(Master_info *mi, Rotate_log_event *rev)
{
  ...
  */
  if (mi->rli.relay_log.description_event_for_queue->binlog_version >= 4)
  {
   /*  
    mysql_mutex_unlock(&mi->data_lock);
    wait_for_all_dml_done(&mi->rli, current_thd->system_thread == SYSTEM_THREAD_SLAVE_SQL);
    mysql_mutex_lock(&mi->data_lock);
   */
    delete mi->rli.relay_log.description_event_for_queue;
  ...
  case XID_EVENT:
	{
	  ...
	  DBUG_PRINT("innosql",("master_last_commit_Pos:%lld",mi->master_last_commit_pos));
	  inc_pos= event_len;
	  break;
	}
  case STOP_EVENT:
    ...
    if (!cur_log->error) /* EOF */
    {
      ...
      */
      ...
      if (relay_log_purge)
      {
        ...
        */
        if (rli->relay_log.purge_first_log(rli, false))
        {
