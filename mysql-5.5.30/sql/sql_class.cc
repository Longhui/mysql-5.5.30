...
THD::~THD()
{
  ...
  if (m_sql_info != NULL)
  {
    MYSQL_DESTORY_SQL_INFO(m_sql_info);
  }
...
/*
  Register that the next commit of this THD should wait to
  ...
  waiter at once during group commit, resolving both of them in the right order.
  ...
  one is registered. But it is ok for serveral waiters to register a wait for
...
wait_for_commit::register_wait_for_prior_commit(wait_for_commit* waitee)
{
  ...
  /*
    ...
    so we need not register. This is nesessary to avoid a race in unregister,
...
/*
  ...
  We have a potential race or deadlock between wakeup_subsequent_commit() in
  ...
  memory data could be accessed. But if we take the two locks in different order
  ...
  is in progress - otherwise the unregister could compele before the wakeup,
  leading to incorrect spurious wakeup or accessing a invalid memory.

  However, if we are in the middle of running wakeup_subsequent_commitd(),then
  there is no need for unregister_wait_for_prior_commit() in the first place - 
  the waiter can just do a normal wait_for_prior_commit(), as it will be immediately
  woken up.
  ...
  wakeup_subsequent_commits() is running, and skip the the registration if
  so. This is needed in case a new waiter would also wait rather than unregister, but it
  would not be woke up until next wakeup, which could be potentially much
