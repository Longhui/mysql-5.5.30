...
void init_update_queries(void)
{
  ...
  */
  ...
  sql_command_flags[SQLCOM_SHOW_PLUGIN_STATUS] = CF_STATUS_COMMAND;
...
case SQLCOM_PREPARE:
  ...
  case SQLCOM_CREATE_TABLE:
  {
    ...
    {
      snprintf(thd->flashback_stmt, sizeof(thd->flashback_stmt), "%c%s.%s:DROP TABLE `%s`.`%s`", 
               '\1', create_table->db, create_table->table_name, create_table->db, create_table->table_name);
    }
  ...
  case SQLCOM_RENAME_TABLE:
  {
    ...
    if (thd->variables.sql_log_flashback)
    {
      snprintf(thd->flashback_stmt, sizeof(thd->flashback_stmt), "%c%s.%s:RENAME TABLE `%s`.`%s` TO `%s`.`%s`", 
               '\1', first_table->next_local->db, first_table->next_local->table_name, first_table->next_local->db, 
               first_table->next_local->table_name, first_table->db, first_table->table_name);
    }
...
create_sp_error:
  ...
  {
    unit->set_limit(select_lex);
    res = inline_mysql_show_sql_stats(unit->select_limit_cnt,thd);
    break;
  }
  case SQLCOM_SHOW_TABLE_STATS:
  {
    res = inline_mysql_show_table_stats(thd);
    break;
  }
  case SQLCOM_SHOW_PLUGIN_STATUS:
  {
    res = inline_mysql_show_plugin_status(thd);
    break;
  }
  case SQLCOM_ANALYZE:
  case SQLCOM_CHECK:
