...
void mysqld_stmt_execute(THD *thd, char *packet_arg, uint packet_length)
{
  ...
  if (stmt->sql_format != NULL && thd->m_sql_info != NULL)
  {
    memcpy(thd->m_sql_info->sql_format, stmt->sql_format, stmt->sql_format_length);
    thd->m_sql_info->sql_byte_count = stmt->sql_format_length;
  }
...
bool Prepared_statement::prepare(const char *packet, uint packet_len)
{
  ...
  */
  ...
  if (thd->m_sql_info)
  {
    thd->m_sql_info->sql_byte_count = 0;
  }
  ...
  if (thd->m_sql_info)
  {
    sql_format_length = thd->m_sql_info->sql_byte_count;
    sql_format = (char *)my_malloc(sql_format_length, MYF(MY_WME));
    memcpy(sql_format, thd->m_sql_info->sql_format, sql_format_length);
