...
bool setup_connection_thread_globals(THD *thd)
{
  if (thd->store_globals())
  {
    ...
    MYSQL_CALLBACK(thd->op_scheduler, end_thread, (thd, 0));
    return 1;                                   // Error
  }
  return 0;
}
...
void do_handle_one_connection(THD *thd_arg)
{
  ...
  if (MYSQL_CALLBACK_ELSE(thd->op_scheduler, init_new_connection_thread, (), 0))
  {
    close_connection(thd, ER_OUT_OF_RESOURCES);
    statistic_increment(aborted_connects,&LOCK_status);
    MYSQL_CALLBACK(thd->op_scheduler, end_thread, (thd, 0));
...
end_thread:
    ...
    if (MYSQL_CALLBACK_ELSE(thd->op_scheduler, end_thread, (thd, 1), 0))
