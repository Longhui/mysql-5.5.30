...
int Rpl_info_table::do_init_info()
{
  ...
  Rpl_info_table_access *access_t = new Rpl_info_table_access;
  THD *thd= access_t->create_thd();

  saved_mode= thd->variables.sql_mode;
  tmp_disable_binlog(thd);

  /*
    Opens and locks the rpl_info table before access_ting it.
  */
  if (access_t->open_table(thd, str_schema, str_table,
  ...
  /*
    ...
    the server_id.
  */
  if ((res= access_t->find_info_for_server_id(KEY_ID, field_idx,
  	                                        field_values, table)) == FOUND_ID)
  {
    ...
    */
    if (access_t->load_info_values(get_number_info(), table->field,
                                 field_values))
      goto end;
  }
  error= (res == ERROR_ID);
  ...
  */
  access_t->close_table(thd, table, &backup, error);
  reenable_binlog(thd);
  thd->variables.sql_mode= saved_mode;
  access_t->drop_thd(thd);
  delete access_t;
  DBUG_RETURN(error);
}
...
int Rpl_info_table::do_flush_info(const bool force)
{
  ...
  */
  											...
  											field_values, table)) == NOT_FOUND_ID)
  {
    ...
    */
    empty_record(table);
    if (access_t->store_info_values(get_number_info(), table->field,
                                  field_values))
    { 
      goto end;
    }
...
int Rpl_info_table::do_check_info()
{
  ...
  */
  ...
  /*
    ...
    reported.
  */
  error = access_t->find_info_for_server_id(KEY_ID, field_idx,field_values, table);
  if(error != FOUND_ID)
  {
    /* 
...
{
  bool res;
  res= (field_values->value[pos].copy(value, strlen(value),
                                        &my_charset_bin));
  return res;
}
...
{
  bool res;
  res= (field_values->value[pos].set_int(value, TRUE,
                                           &my_charset_bin));
  return res; 
}
...
{
  bool res;
  res= (field_values->value[pos].set_int(value, FALSE,
                                           &my_charset_bin));
  return res;
}
...
{ 
  bool res;
  res= (field_values->value[pos].set_real(value, NOT_FIXED_DEC,
                                            &my_charset_bin));
  return res;
}
...
bool Rpl_info_table::do_is_transactional()
{
  ...
  */
  ...
  access_t->close_table(thd, table, &backup, 0);
  thd->variables.sql_mode= saved_mode;
  access_t->drop_thd(thd);
  delete access_t;
