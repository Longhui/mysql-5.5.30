...
static int commit_one_phase_low(THD *thd, bool all, THD_TRANS *trans, bool is_real_trans);
...
/*
  Invoke commit_checkpoint_request() in all storage engine that implement it.
...
commit_one_phase_low(THD *thd, bool all, THD_TRANS *trans, bool is_real_trans)
{
  ...
  DBUG_ENTER("ha_commit_one_phase_low");
...
{
  handlerton *hton= plugin_data(plugin, handlerton *);
  /*external xa commit called by log::commit_by_xid, need not do commit in InnoDB*/
...
int ha_commit_or_rollback_by_xid(XID *xid, bool commit)
{
  ...
  {
    plugin_foreach(NULL, xaregister_handlerton,
                                    ...
                                    void *arg)
{
  ...
  if(wait_rollback_ok && (NULL != hton->rollback_ok) )
  {
    while (!hton->rollback_ok(hton))
    {
      my_sleep(100000);
    }
  }
...
#if 0 && !defined(WILL_BE_DELETED_LATER)
  /*
    for now, only InnoDB supports 2pc. It means we can always safely
    rollback all pending transactions, without risking inconsistent data
  */
  /* Innodb & TokuDB & TNT and binlog totally 4 2pc engines */
