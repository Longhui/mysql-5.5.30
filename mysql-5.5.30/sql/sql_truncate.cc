...
bool bak_table(TABLE_LIST *table)
{
  ...
  */
  ...
  if (!thd->is_error())
  {
    snprintf(buf, sizeof(buf), "create table `%s`.`%s` like `%s`.`%s`;", table->db, table->table_name, FLASHBACK_DB, new_name_buff);
...
bool Truncate_statement::execute(THD *thd)
{
  ...
  if (thd->variables.sql_log_flashback && bak_table(first_table))
    return FALSE;
