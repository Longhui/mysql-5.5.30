...
/*
  ...
  DESCRIPTION
    This is a very long function and is everything but the kitchen sink :)
    ...
    if (!error && (new_name != table_name || new_db != db))
    {
      ...
      */
      ...
      else
      {
        ...
        {
          snprintf(thd->flashback_stmt, sizeof(thd->flashback_stmt), "%c%s.%s:RENAME TABLE `%s`.`%s` TO `%s`.`%s`", '\1', new_db, new_alias, new_db, new_alias, db, table_name);
        }
...
copy_table:
  ...
  */
  ...
  if (!error)
  {
    if (need_copy_table != ALTER_TABLE_DATA_CHANGED || !thd->variables.sql_log_flashback)
    {
    ...
    else
    {
      ...
      snprintf(&thd->flashback_stmt[len + 1], sizeof(thd->flashback_stmt) - len - 1, "%s.%s:DROP TABLE `%s`.`%s`", db, new_name, db, new_name);

      (void) mysql_rename_table(old_db_type, db, old_name, FLASHBACK_DB, new_name_buff, FN_FROM_IS_TMP);
    }
  }
