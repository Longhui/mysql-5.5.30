...
/**
  Replication binlog relay SQL observer parameter
*/
typedef struct Binlog_relay_SQL_param {
  uint32 server_id;

  char *master_log_name;
  my_off_t master_log_pos;

} Binlog_relay_SQL_param;

/**
   Observes and extends the service of slave SQL thread.
*/
typedef struct Binlog_relay_SQL_observer {
  uint32 len;
    /**
     This callback is called after a event from relaylog apply
     ...
     @retval 1 Failure
  */
  int (*after_apply_event)(Binlog_relay_SQL_param *param);
	
} Binlog_relay_SQL_observer;
...
/**
   Register a binlog relay IO (slave SQL thread) observer

   @param observer The binlog relay SQL observer to register
   @param p pointer to the internal plugin structure
...
/**
   Unregister a binlog relay SQL (slave SQL thread) observer

   @param observer The binlog relay SQL observer to unregister
   @param p pointer to the internal plugin structure
