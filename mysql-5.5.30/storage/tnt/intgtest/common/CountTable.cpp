 ...
 * @return 表定义
 */
const TableDef* CountTable::getTableDef(bool useMms) {
	if (useMms)
 ...
 * @return 创建好的记录
 */
Record* CountTable::createRecord(u64 id, int count) {
	RecordBuilder rb(CountTable::getTableDef(true), RID(0, 0), REC_REDUNDANT);
 ...
 * @return 记录数
 */
uint CountTable::populate(Session *session, Table *table, u64 *dataSize, u64 startId) {
	Record *rec;
...
* @return 记录数
*/
uint CountTable::populate(Session *session, Table *table, u64 *dataSize, u64 maxId, u64 minId) {
	u64 volumnCnt = *dataSize;
	uint recCnt = 0;
	uint dupIdx;
	int startId = 0;
 ...
 */
TableDef* CountTable::makeTableDef(bool useMms) {
	TableDefBuilder *builder;
	if (useMms)
		builder = new TableDefBuilder(TableDef::INVALID_TABLEID, "perfTest", "CountUseMms");
	else
		builder = new TableDefBuilder(TableDef::INVALID_TABLEID, "perfTest", "CountNotUseMms");
 ...
 * @return 更新之后的记录
 */
Record* CountTable::updateId(Record *rec, u64 id) {
 ...
 * @return 更新之后的记录
 */
Record* CountTable::updateCount(Record *rec, int count) {
	assert(rec->m_format == REC_REDUNDANT);
	const TableDef *tableDef = getTableDef(false);
 ...
 * @return 更新之后的记录
 */
Record* CountTable::updateRecord(Record *rec, u64 id, int count) {
	assert(rec->m_format == REC_REDUNDANT);
	const TableDef *tableDef = getTableDef(false);
 ...
 * @return 更新之后的key
 */
SubRecord* CountTable::updateKey(SubRecord *key, u64 id) {
 ...
 * @return 扫描过的记录数
 */
u64 CountTable::scanTable(Database *db, Table *table) {
