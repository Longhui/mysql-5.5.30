...
void MmsReplacePolicyTest::setUp() {
	...
	stringstream name;
	for (int i = 0; i < m_numTables; i++) { // Count表
		name.str("");
		name << i;
		try {
...
void MmsReplacePolicyTest::warm() {
	...
	for (int i = 0; i < m_numTables; i++)
		m_rowIdTables[i] = new RowId[m_recCount];   // 记录的最小极限情况为一个bigint
	Connection *conn = m_db->getConnection(false);
	...
	for (int i = 0; i < m_numTables; i++)
		m_zipfRandoms[i] = ZipfRandom::createInstVaryScrew(m_recCount, false); // 创建Prop(Top10%) = 90% 的Zipf随机对象
