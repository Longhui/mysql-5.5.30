...
u32 TNTTableScanTest::VOLUMN_RATIO = 100;
 ...
 */
TNTTableScanTest::TNTTableScanTest(bool useMms, bool recInMms, bool isVar)
{
	m_useMms = useMms;
	m_recInMms = recInMms;
	m_isVar = isVar;
 ...
 * @return 测试用例名
 */	    
string TNTTableScanTest::getName() const
{
	stringstream ss;
	ss << "TNTTableScanTest(";
	if (m_useMms)
 ...
 * @return 测试用例描述信息
 */
string TNTTableScanTest::getDescription() const
{
	return "TNTTableScan in Memory Performance";
}
 ...
 * @return 缓存名
 */
string TNTTableScanTest::getCachedName() const	
{
    return string("TNTTableScanTest-") + m_tableDef->m_name;
}
 ...
 * @post 保持数据库为关闭状态
 */
void TNTTableScanTest::loadData(u64 *totalRecSize, u64 *recCnt)
{
	openTable(true);
...
void TNTTableScanTest::run()
{
	m_opCnt = TNTAccountTable::scanTable(m_db, m_table);
	m_opDataSize = m_totalRecSizeLoaded; // 操作数据量 = 表数据量
}
void TNTTableScanTest::setUp()
{
	// 开启测试数据缓存
	m_enableCache = true;
	// 内存测试用例
	...
	// 初始表数据量
	m_totalRecSizeLoaded = m_config->m_ntseConfig.m_pageBufSize * Limits::PAGE_SIZE / TNT_TABLE_SCAN_REDUCE_RATIO;
	if (m_useMms && m_recInMms)
		m_config->m_ntseConfig.m_mmsSize = m_totalRecSizeLoaded * TNT_TABLE_SCAN_MMS_MULTIPLE / Limits::PAGE_SIZE;
}