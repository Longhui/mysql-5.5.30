...
loop:
	...
	if (nth_lock == 0) {
		...
		trx_print(file, trx, 600);
		/*k = 0;
		n_lock = UT_LIST_GET_LEN(trx->trx_locks);
		lock1   = UT_LIST_GET_FIRST(trx->trx_locks);
		while (k < n_lock)
		{
			if (lock_get_type_low(lock1) == LOCK_REC)
			{
				lock_rec_print(file, lock1);
			}
			else
			{
				lock_table_print(file, lock1);
			}
			putc('\n', file);
			lock= UT_LIST_GET_NEXT(trx_locks, lock1);
			k++;
		}*/
		if (trx->read_view) {
			fprintf(file,
