...
static bool innobase_rollback_ok(handlerton *hton);
	...
	void	*p)	/*!< in: InnoDB handlerton */
{
	...
	innobase_hton->rollback_ok=innobase_rollback_ok;
...
/*****************************************************************//**
Handler a commit checkpoint request from server layer.
...
static MYSQL_SYSVAR_BOOL(change_buffer_count_size, 
  srv_ibuf_count_buffer_pages, PLUGIN_VAR_NOCMDARG,
  "Whether to use counting size of ibuf page in buffer pool.",
  NULL, NULL, FALSE);
...
*/
  ...
  MYSQL_SYSVAR(file_flush_sleep_time),
  ...
  MYSQL_SYSVAR(change_buffer_count_size),
#if defined UNIV_DEBUG || defined UNIV_IBUF_DEBUG
...
static
bool innobase_rollback_ok(handlerton *hton)
{
	trx_t* trx;
	DBUG_ASSERT(hton == innodb_hton_ptr);
	mutex_enter(&kernel_mutex);
