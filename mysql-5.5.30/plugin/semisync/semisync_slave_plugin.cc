...
int repl_semi_slave_apply_event(Binlog_relay_SQL_param *param)
{
  (void) repl_semisync.reportApplyEvent(param->master_log_name, param->master_log_pos);
  return 0;
}
				...
				uint32 flags)
{
  if (rpl_semi_sync_slave_status && semi_sync_need_reply)
  {
    ...
    (void) repl_semisync.waitApply(param->master_log_name,
...
static void fix_rpl_semi_sync_slave_fullsync(MYSQL_THD thd,
					    SYS_VAR *var,
					    void *ptr,
					    const void *val)
{
  *(char *)ptr= *(char *)val;
  repl_semisync.setFullsync(rpl_semi_sync_slave_fullsync != 0);
  return;
}
...
static void fix_rpl_semi_sync_slave_timeout(MYSQL_THD thd,
						SYS_VAR *var,
						void *ptr,
						const void *val)
{
  *(unsigned long *)ptr= *(unsigned long *)val;
  repl_semisync.setWaitTimeout(rpl_semi_sync_slave_timeout);
  return;
}

static int rpl_semi_sync_slave_show_fullsync(MYSQL_THD thd, SHOW_VAR *var, char *buff)
{
  repl_semisync.setExportVals();
  var->type= SHOW_BOOL;
  var->value= (char*)&rpl_semi_sync_slave_fullsync_status;
  return 0;
}

/* plugin system variables */
static MYSQL_SYSVAR_BOOL(fullsync, rpl_semi_sync_slave_fullsync,
  PLUGIN_VAR_OPCMDARG,
 "Enable IO thread be synchronous with SQL thread (disabled by default). ",
  NULL, // check
  &fix_rpl_semi_sync_slave_fullsync, // update
  0);
...
static MYSQL_SYSVAR_ULONG(timeout, rpl_semi_sync_slave_timeout,
  PLUGIN_VAR_OPCMDARG,
  "The timeout value (in ms) for full-synchronous replication in the slave",
  NULL,
  &fix_rpl_semi_sync_slave_timeout, //update
  30000, 0, ~0UL, 1);

static SYS_VAR* semi_sync_slave_system_vars[]= {
  MYSQL_SYSVAR(enabled),
  MYSQL_SYSVAR(trace_level),
  MYSQL_SYSVAR(fullsync),
...
static SHOW_VAR semi_sync_slave_status_vars[]= {
  ...
  {"Rpl_semi_sync_slave_wait_num",
   (char*) &rpl_semi_sync_slave_sql_wait_num, SHOW_LONGLONG},
  {"Rpl_semi_sync_slave_wait_time",
   (char*) &rpl_semi_sync_slave_sql_wait_time, SHOW_LONGLONG},
  {"Rpl_semi_sync_slave_wait_timeout",
   (char*) &rpl_semi_sync_slave_sql_wait_timeouts, SHOW_LONGLONG},
  {"Rpl_semi_sync_slave_wait_fail",
   (char*) &rpl_semi_sync_slave_sql_wait_fail, SHOW_LONGLONG},
  {"Rpl_semi_sync_slave_fullsync_status",
   (char*) &rpl_semi_sync_slave_show_fullsync, SHOW_FUNC},
  {NULL, NULL, SHOW_BOOL},
};

Binlog_relay_SQL_observer relay_sql_observer= {
  sizeof(Binlog_relay_SQL_observer), //len
  
  repl_semi_slave_apply_event, //after_apply_event
};
...
PSI_mutex_key key_ss_mutex_LOCK_relaylog_;

static PSI_mutex_info all_fullsync_mutexs[]=
{
  {&key_ss_mutex_LOCK_relaylog_,"LOCK_relaylog_",0}
};

PSI_cond_key key_ss_cond_COND_relaylog_queue_;

static PSI_cond_info all_fullsync_conds[]=
{
  {&key_ss_cond_COND_relaylog_queue_,"COND_relaylog_queue_",0}
};

static void init_fullsync_psi_keys(void)
{
  const char* category= "fullsync";
  int count;

  if(PSI_server == NULL)
  	return;

  count= array_elements(all_fullsync_mutexs);
  PSI_server->register_mutex(category,all_fullsync_mutexs,count);

  count= array_elements(all_fullsync_conds);
  PSI_server->register_cond(category,all_fullsync_conds,count);
}
...
static int semi_sync_slave_plugin_init(void *p)
{
  ...
  if (register_binlog_relay_io_observer(&relay_io_observer, p))
    return 1;
  if (register_binlog_relay_sql_observer(&relay_sql_observer, p))
    return 1;
  return 0;
}
...
{
  unregister_slave_observer();
  if (unregister_binlog_relay_io_observer(&relay_io_observer, p))
    return 1;
  if (unregister_binlog_relay_sql_observer(&relay_sql_observer, p))
    return 1;
  return 0;
}
