				  ...
				  my_off_t trx_wait_binlog_pos)
{
  ...
  if (getMasterEnabled() && trx_wait_binlog_name)
  {
        ...
        if (cmp <= 0)
	{
      ...
      abstime.tv_nsec= start_ts.tv_nsec + (unsigned long long)(wait_timeout_ % TIME_THOUSAND)*TIME_MILLION;
